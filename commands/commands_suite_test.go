...
const SECRET_CERTIFICATE_REQUEST_JSON = `{"type":"certificate","credential":{"ca":"%s","certificate":"%s","private":"%s"}}`
const SECRET_CERTIFICATE_RESPONSE_JSON = `{"type":"certificate","credential":{"ca":"%s","certificate":"%s","private":"%s"},"updated_at":"` + TIMESTAMP + `"}`
const SECRET_CERTIFICATE_RESPONSE_TABLE = `Type:		certificate\nName:		%s\nCA:		%s\nCertificate:		%s\nPrivate:	%s\nUpdated:	` + TIMESTAMP
const CA_REQUEST_JSON = `{"type":"%s","root":{"certificate":"%s","private":"%s"}}`
const CA_RESPONSE_JSON = `{"type":"%s","root":{"certificate":"%s","private":"%s"},"updated_at":"` + TIMESTAMP + `"}`
